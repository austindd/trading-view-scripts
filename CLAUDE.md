...
| `supply-demand-zones/market-structure-supply-demand-v3.ps` | ~690 | **V2 engine + display features.** Adds an option to print each zone candle's aggregated volume inside the zone (as box text — left edge, vertically centered, no extra drawing objects), and a "Top Volume Zones Per Side" render filter that shows only the N highest-volume active zones per side, with an optional chart-bar cutoff limiting how old competing zones may be. Filtering affects rendering only; all zones still tracked and alerted. |
